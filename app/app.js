...
/* ---- 2-day demo window: ALL options unlocked today + tomorrow, then locks ----
   The demo starts the first time the app is opened; it stays open for that day and
   the next, then shows the "upgrade to use the app" screen. */
const DEMO_DAYS = 2;
function demoStartISO(){
  try{ let s=localStorage.getItem('dnk_demo_start'); if(!s){ s=todayISO(); localStorage.setItem('dnk_demo_start',s); } return s; }
  catch(e){ return todayISO(); }
}
function demoEndISO(){ return addDaysISO(demoStartISO(), DEMO_DAYS-1); }
function demoActive(){ return todayISO() <= demoEndISO(); }
function demoDaysLeft(){ return Math.max(0, diffDaysISO(demoEndISO(), todayISO())+1); }

const ROLE_PERMS = {
  Admin:   {dashboard:1,newinvoice:1,invoices:1,payments:1,leads:1,concalc:1,revenue:1,inventory:1,staff:1,customers:1,sites:1,vehicles:1,grades:1,rates:1,reports:1,users:1,settings:1},
  Manager: {dashboard:1,newinvoice:1,invoices:1,payments:1,leads:1,concalc:1,revenue:1,inventory:1,staff:1,customers:1,sites:1,vehicles:1,grades:1,rates:1,reports:1,users:0,settings:0}
};
...
function seed(){
  return {
    ...
    staff:[
      {id:'st1',name:'Ramesh K',role:'Driver',phone:'9012345678',wage:650,active:true},
      {id:'st2',name:'Suresh M',role:'Driver',phone:'9034567890',wage:650,active:true},
      {id:'st3',name:'Anjaneyulu',role:'Batching Operator',phone:'9701122334',wage:800,active:true},
      {id:'st4',name:'Lakshmi',role:'Office / Accounts',phone:'9885566778',wage:700,active:true},
      {id:'st5',name:'Venkatesh',role:'Loader',phone:'9012000123',wage:550,active:true},
    ],
    attendance:[],
    seq:1402,
...
function migrate(d){
  const s=seed();
  ['grades','customers','sites','vehicles','rates','invoices','payments','users','leads','products','stockmoves','staff','attendance'].forEach(k=>{ if(!Array.isArray(d[k])) d[k]=s[k]; });
...
const routes = {
  ...
  inventory:renderInventory, staff:renderStaff
};
...
function logout(){ loggedIn=false; ME=null; authSet(false); meSet(null); renderApp(); }
...
function renderApp(){
  if(!demoActive()){ renderDemoEnded(); return; }
  ...
  <div class="app">
    <div class="sidebar">
      ...
      <div class="demoband">⏳ DEMO • ${left} day${left===1?'':'s'} left</div>
...
/* Demo expiry screen — shown after the 2-day window */
function renderDemoEnded(){
  document.getElementById('root').innerHTML=`
  <div class="demo-end"><div class="card">
    <img src="${window.LOGO_DATA}" alt="DNK Power Conmix">
    <h1>Demo Period Has Ended</h1>
    <p>Your <b>2-day free demo</b> of the DNK RMC Billing &amp; Plant ERP has ended.<br>
       Upgrade to continue using the app with all features and your saved data.</p>
    <a class="btn gold" style="justify-content:center;width:100%;margin-top:18px" href="mailto:helpdesk@epixs.in?subject=DNK%20RMC%20-%20Upgrade%20to%20Full%20Version&body=We%20would%20like%20to%20upgrade%20the%20DNK%20RMC%20app%20to%20the%20full%20version.">Upgrade to Use the App →</a>
    <div class="dcontact">
      EPIXS Media &nbsp;•&nbsp; helpdesk@epixs.in &nbsp;•&nbsp; +91 94411 09327<br>
      <a href="https://dnk-rmc-proposal.web.app" target="_blank" rel="noopener">View proposal &amp; packages →</a>
    </div>
  </div></div>`;
}
...
/* ---- Users & Permissions ---- */
const PERM_LABELS={dashboard:'Dashboard',newinvoice:'New Bill',invoices:'Invoices',payments:'Outstanding',inventory:'Product Inventory',staff:'Staff Attendance',leads:'Leads',concalc:'Concrete Calc',revenue:'Revenue Calc',customers:'Customers',sites:'Sites',vehicles:'Vehicles',grades:'Grades',rates:'Rates',reports:'Reports',users:'Users',settings:'Settings'};
...
function renderInventory(){
  ...
  document.getElementById('main').innerHTML=
    topbar('Product Inventory','Raw materials &amp; stock &mdash; cement, aggregate, admixtures and more',
      `<button class="btn gold" onclick="prodModal()">➕ Add Product</button>`)+
...
function delProd(id){ if(confirm('Delete this product?')){ DB.products=DB.products.filter(p=>p.id!==id); save(); renderInventory(); } }
...
function saveMove(type,id){
  ...
  DB.stockmoves.push({id:uid('sm'),productId:id,type,qty,date:val('mv_date')||todayISO(),note:val('mv_note')});
...
function renderStaff(){
  ...
  document.getElementById('main').innerHTML=
    topbar('Staff Attendance','Mark daily attendance and view the monthly summary',
      `<button class="btn gold" onclick="staffModal()">➕ Add Staff</button>`)+
    `<div class="toolbar">
...
function staffModal(id){
  const s=id?staffById(id):{name:'',role:'',phone:'',wage:'',active:true};
  modal((id?'Edit':'Add')+' Staff',
    `<div class="form-grid">
      <div class="field"><label>Name *</label><input id="sf_name" value="${s.name||''}"></div>
      <div class="field"><label>Role / Designation</label><input id="sf_role" value="${s.role||''}" placeholder="Driver / Operator / Loader"></div>
      <div class="field"><label>Phone</label><input id="sf_phone" value="${s.phone||''}"></div>
      <div class="field"><label>Wage per day (₹)</label><input id="sf_wage" type="number" value="${s.wage!==''?s.wage:''}"></div>
      <div class="field full"><label>Status</label><select id="sf_active"><option value="1" ${s.active!==false?'selected':''}>Active</option><option value="0" ${s.active===false?'selected':''}>Inactive</option></select></div>
    </div>`,
...
function saveStaff(id){
  const o={name:val('sf_name'),role:val('sf_role'),phone:val('sf_phone'),wage:Number(val('sf_wage'))||0,active:val('sf_active')==='1'};
  if(!o.name)return toast('Name required','err');
  if(id){ Object.assign(staffById(id),o); } else { DB.staff.push({id:uid('st'),...o}); }
  save(); closeModal(); toast('Staff saved','ok'); renderStaff();
}
function delStaff(id){ if(confirm('Delete this staff member?')){ DB.staff=DB.staff.filter(s=>s.id!==id); save(); renderStaff(); } }
