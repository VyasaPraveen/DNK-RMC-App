...
function invoiceHTML(inv, company, opts){
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    /* The document sizes to its content (never forced to full page height) so the
       bank details, signature and declaration always sit well inside the printable
       area and never fall into the printer's non-printable bottom margin. */
    .doc{border:1px solid #000}
    /* A small, fixed breathing gap before the footer block — enough to separate it
       from the tax section without pushing it down to the page edge. */
    .fill{height:20px}
