...
function invoiceHTML(inv, company, opts){
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    /* Fill the A4 page: the framed document stretches down the page and a flexible
       spacer (.fill) pushes the bank/signature block toward the bottom. min-height
       is held ~30mm under the full page so the declaration & footer always keep a
       safe clearance from the printer's non-printable bottom edge and never clip. */
    .doc{border:1px solid #000;display:flex;flex-direction:column;min-height:calc(297mm - 30mm)}
    .fill{flex:1 1 auto}
    ...
    /* Where two tables stack directly (header -> items), drop the lower table's top
       border so their junction is a single 1px line, never a doubled 2px one. */
    .doc table+table>tbody>tr:first-child>td,.doc table+table>tbody>tr:first-child>th{border-top:0}
    ...
    /* Keep each bank-detail label on a single line (e.g. "Bank Name") so every
       label:value pair stays on one row with the colons aligned. */
    .bank td:first-child{white-space:nowrap}
    .sign{height:60px}
      ...
      <tr>
        <td style="width:50%" class="bank">
          ...
          <table style="margin-top:3px"><tbody class="bank">
            <tr><td style="width:84px">Bank Name</td><td>: ${esc(company.bank.bank)}</td></tr>
